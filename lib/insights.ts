...
export const INSIGHTS: Insight[] = [
  {
    ...
    title: "The harness is the product. Here is its input/output spec.",
    dek: "Nine in ten agent pilots never reach production, and the engineering is rarely what fails — the inputs are missing. Every brief, journey map, and measurement plan a strategist has ever written is a named component of the agent harness. Each one comes back out as a number somebody can audit.",
    metaDescription: "Only 11% of organizations have an agent in production. The agent harness has an input spec — and fifteen years of strategy deliverables already wrote it.",
    pillar: 'Build',
    lens: 'Build',
    ...
    body: [
      "An agent harness is everything in an agent that is not the model — prompt construction, tools, memory, permission boundaries, validators, and the loop that decides whether any of it worked. It is also where every deliverable a strategist has ever produced goes to become executable.",
      "![The Campanile at UC Berkeley at dusk with San Francisco and the Bay beyond — Part 2 of The Forward Deployed Strategist, on the input/output spec of the agent harness](/images/insights/agent-harness-inputs-outputs-hero.png)",
      "[Part 1](https://ensolabs.ai/insights/forward-deployed-strategist-agency-lineage) argued the scarce half of forward deployment is the strategist seat. This is the follow-through: what that seat hands over, and what comes back.",
      "## The market is hiring one half of the role",
      "![Demand versus delivery, animated — U.S. forward-deployed postings rising from 643 in April 2025 to 5,330 in April 2026, up 729%, against just 11% of organizations with an agent actually in production](/images/insights/agent-harness-demand-delivery-anim.gif)",
      "Postings for the engineering half went up roughly 8x in a year. The share of companies that actually ship a system did not move with it. That gap is the whole story.",
      "## The crossing almost nobody makes",
      "![The crossing, animated — 38% of organizations piloting agents against 11% in production, with evaluation gaps at 64%, governance friction at 57%, model reliability at 51%, and 171% average ROI for the agents that do cross](/images/insights/agent-harness-gap-anim.gif)",
      "Not one of those three blockers is a model-capability problem. All three are harness problems, and each traces back to an input nobody wrote down before the build began. The agents that do cross return an average [171% ROI](https://www.digitalapplied.com/blog/agentic-ai-statistics-2026-definitive-collection-150-data-points).",
      ...
      "## The input spec",
      "Brand, digital, CX, and data strategy produce four artefacts on repeat: a brief, a journey map, a measurement plan, a segmentation. In an agentic system none of them are background documents. Each is a component with a slot.",
      "![The input/output spec, animated — the brief becomes the system prompt and permission boundary, the journey map becomes the tool allowlist and escalation gate, the measurement plan becomes the eval harness and golden-set CI, and the segmentation becomes the retrieval strategy and recall target](/images/insights/agent-harness-inputs-outputs-pipeline.gif)",
      "Journey mapping was always a measurement discipline wearing a design costume. You were documenting where value was created, where it leaked, and which decision at which moment would have changed the outcome. Those are exactly the decisions an agent has to encode.",
      "## Three tiers, one of which is now a legal surface",
      "![The harness in three tiers, animated — Runtime, Capabilities and Assurance, with the obligations regulated deployment adds to each: audit trails and encryption, NIST AI RMF and FDA/MLR governance of the tool list and corpus, and policy-as-code with human approval before anything irreversible](/images/insights/agent-harness-stack-anim.gif)",
      "In a regulated deployment every tier picks up an obligation, and the escalation rule stops being a design preference. It becomes the control an auditor asks to see.",
      "## The output spec",
      "![Same model, different harness — Microsoft's AgenticRAG results, animated: BRIGHT recall@1 rising from a 27.8% embedding baseline to 49.6%, and 92% answer correctness on FinanceBench against a 94% oracle ceiling](/images/insights/agent-harness-benchmark-anim.gif)",
      "Microsoft's [AgenticRAG](https://arxiv.org/abs/2605.05538) paper held the model constant and changed how the system was allowed to look for evidence. That is a retrieval-strategy decision, made upstream, by whoever defined what \"relevant\" means for the business. The harness moved the number.",
      "::stat\n49.6% | recall@1 on BRIGHT, +21.8pp over the best embedding baseline\n92% | answer correctness on FinanceBench, within 2pp of oracle evidence\n11% | of organizations with an agent actually in production\n171% | average ROI on the agents that reach production",
      "Retrieval quality, faithfulness, tool-selection accuracy, escalation rate — this is the measurement plan, executed. A strategist who could defend a media-mix model to a CFO can defend these. Same object, new stack.",
      "## How we build",
      "For a Fortune 500 manufacturer we encoded a lead scientist's relevance rules — what counts as a real commercial signal, what is noise — before a single pipeline stage was written. For a pharma AI Center of Excellence, the claims and governance layer was written first and the agents were built inside it.",
      "Same sequence both times, and it is the only part worth copying: specify the inputs, then build the system to satisfy them. Reverse the order and you land in the 89%.",
      "## What to do about it",
      "- [ ] Write the escalation rule before the architecture. If nobody can say where the agent stops and a human starts, it is not scoped — it is a demo with a roadmap.",
      "- [ ] Name the one number that would prove this worked, and put it in the eval harness on day one. If it cannot be measured on a golden set, it is not a metric, it is a hope.",
      "- [ ] Judge the data before the model. Retrieval strategy moved the benchmark more than model choice did. Your corpus is the variable you actually control.",
