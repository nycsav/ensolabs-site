...
- **🎯 CAREER-ROI LENS — the primary scoring/prioritization rule (set 2026-07-13; overrides generic "frontier fit").** Sav's #1 objective right now: land a client/contract or a full-time role, break into the **GTM & growth-marketing** niche, and convert his **15 years of Madison Avenue advertising + data-analytics** into AI-native GTM. Every surfaced event must translate to **tangible pipeline or a sellable skill** — not just frontier proximity. Score each on 4 axes (0–10) and lead with the composite: (1) **Buyer density** (decision-makers/clients/referrers in the room), (2) **GTM/Growth fit**, (3) **Career leverage** (contract/warm intro/job lead), (4) **Skill-to-sell** (packageable into an Enso offer). Enso Fit v2 = secondary tiebreak. Prioritize: buyer/GTM-leader rooms > growth/GTM-engineering sessions > frontier rooms that yield intros > pure technical builds/all-day hackathons (deprioritize unless a networking play). For the top pick, always add **who to meet (named) + one opener**. Positioning line to weave in: *"15 years turning data into brand growth on Madison Avenue → now building & operating agentic GTM & growth systems at Enso Labs."* This lens is encoded in the `daily-event-scan` prompt and applies to `weekly-event-board-sweep` + `hackathon-radar-weekly` too.
...
- **Workstream color convention on the PRIMARY calendar (set 2026-07-13 per Sav — he chose "primary + color convention" over a separate calendar).** Claude's Google tools can create/update/delete calendar *events* but CANNOT create a new Google Calendar (`calendars.insert` not exposed), so all workstreams live on the primary calendar, distinguished by color so Sav can read his work days/weeks at a glance:
  - **Events (Enso Labs networking/frontier)** keep the Enso Fit v2 scheme — 🟢 Basil/green `colorId 10` (approved, 8+), 🟡 Banana/yellow `colorId 5` (approved, 6–7.5), ⚪ Graphite/gray `colorId 8` (pending·waitlist·payment-pending·≤5). Title carries ✅/⏳/⬜ + ⭐score.
  - **Heller / client delivery** → 🍅 Tomato `colorId 11`. **Job search / interviews (incl. Perplexity)** → 🫐 Blueberry `colorId 9`. **Enso Labs build / deep-work blocks** → 🌊 Peacock `colorId 7`. **Personal / life admin** → 🍇 Grape `colorId 3`.
  - Convention applies going forward; `daily-event-scan` keeps events colored as above. Non-event items are colored when Claude creates/edits them (retroactive backfill only if Sav asks).
