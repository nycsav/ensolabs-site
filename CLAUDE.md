...
- **Event inbox cleanup runs after EVERY event-scan run (standing rule — set 2026-07-13 per Sav).** Whenever `daily-event-scan` runs (3x/day, 8a/1p/6p ET), it also files that run's new event emails into the Gmail "Events" label (`Label_8`) and archives them, so the inbox stays clean through the day — not just at night. The dedicated `events-inbox-filer` was raised to 4x/day (8a/1p/6p/9p ET) to match this cadence + a nightly backstop. NEW filing rule (reverses the old "keep pending/waitlist visible"): **MOVE pending + confirmed + waitlist + update + cancellation emails OUT** of the inbox into "Events" + archive; keep in inbox ONLY true action-required (a real person's 1:1 invite awaiting Sav's yes/no, a payment step, or a message needing a reply). Never touch client/job/billing/personal/bot mail; never delete. Tool: `mcp__enso-google__gmail_modify_labels(message_id, add_label_ids=["Label_8"], remove_label_ids=["INBOX"])`.
